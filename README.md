...
### 1. Détection de fingerprinting (+ empoisonnement optionnel)
...
**Protection par empoisonnement (opt-in, par site).** Plutôt que bloquer les scripts de fingerprinting — souvent logés dans du code vital, donc cassants — l'extension peut **fausser** ce qu'ils lisent : bruit imperceptible sur le canvas, mensonge sur le modèle GPU, bruit inaudible sur l'audio. Le site tourne normalement ; l'empreinte récoltée est fausse et inexploitable. Activation dans le popup (« Protéger contre le fingerprinting »), puis rechargement.
- Le bruit est **déterministe par session** (empreinte stable-mais-fausse, plus crédible qu'un bruit changeant).
- Une page ne peut **pas désactiver** la protection (seule l'activation est honorée).
- Se protéger peut soi-même devenir un signal distinctif (« utilise un anti-fingerprinting ») — paradoxe assumé, d'où l'opt-in par site.
